...
class Barcode
{
   ...
   public function __construct ( $number, $scale, $print = false, $fontpath=null)
   {
      ...
      $this->_bars = $this->_encode(); // encode numbers into binary
      $this->_createImage(); // prepare image
      $this->_drawBars(); // draw the bars
      $this->_drawText(); // put text under bars, but only if print switch is true
   }
   ...
   {
      $this->_height = $this->scale * 50;
      //$this->_width  = 1.8 * $this->_height;
      $this->_width  = 1.8 * $this->scale * 60;
   ...
   {
      $x = $this->_width*0.032;
      $y = $this->_height*0.975;

      $text_color=ImageColorAllocate($this->_image, 0x00, 0x00, 0x00);

      $fontsize = $this->scale*6.5;
      $kerning = $fontsize*1.087;
