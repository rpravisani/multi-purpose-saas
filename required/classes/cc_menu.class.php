...
class cc_menu{
	...
	function __construct($active_page=false, $page_permissions=false, $db=false, $extra = false){
		...
		if(!$page_permissions) die("No page permitted!"); // todo better error handeling!
	...
	public function prepareMenu(){
		...
		foreach($this->parent_pages as $i=>$page){
			$parentUrl = ($page['type'] == "label") ? "" : $this->setUrl($page['id']);
			$parentClass = ($page['type'] == "label") ? "header ".$page['tag_class'] : false;
			...
			if($children){ // ok got children
				...
				foreach($children as $c=>$child){ // loop children
					...
					// set child item class to active if current page corripondes to child item
					...
					$childItem .= ">";
					$childItems[$c] = $childItem."<a href=\"".$childUrl."\"><i class=\"fa fa-".$childicon."\"></i> ".$child['name'];
					if(!empty($child['tag'])) $childItems[$c] .= "<span class=\"label pull-right ".$child['tag_class']."\">".$child['tag']."</span>\n"; // tag
					$childItems[$c] .= "</a></li>\n";
				}  // end foreach
				...
				if(!empty($page['tag'])) $parentItem .= "<sup class=\"label ".$page['tag_class']."\">".$page['tag']."</sup>\n"; // tag
				$parentItem .= "  </a>\n";
			...
			if($page['type'] == "label"){
				...
				if($parentItem){ // if parent html is already set (created in the got children subroutine), add it to the array of the menu item
					...
					$parentItems[$i] .= "    <i class=\"fa fa-".$page['icon']."\"></i> <span>".$page['name']."</span>"; // icon
	...
	// set a disissable warning message for comments or warnings - added here cause no page class (yet)
	public function pageMessage($title, $message, $whom = false){ // $whom => array of user's how can view these
		if($whom){
			if(!is_array($whom)) $whom = array ( $whom );
			if(!in_array($_SESSION['login_type'], $whom)) return false;
		}
		$_SESSION['page_message_title'] = $title;
	...
	private function setUrl($pid){
		...
		if(!empty($page_data['fraction'])) $url .= "#".$page_data['fraction']; // add action if any defined (insert, update or delete)
		return $url;
	}
